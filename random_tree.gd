...
func _get_new_tree():
	if get_child_count() > 0:
		_clear_tree( self.get_children()[0] )
	trunc = null
	_generate_tree()
...
func _generate_tree():
	...
	p.set_pos( base )
	add_child( p )
	trunc = Branch.new( null, p, a, base_width, top_width, height )
...
func _recursive_branch( p, orientation, layer, on_top = false ):
	...
	br.rotate( a )
	br.set_pos( base )
	p.polygon2d.add_child( br )
	
	var branch = Branch.new( p, br, a, base_width, top_width, length )
	p.add_child( branch )
